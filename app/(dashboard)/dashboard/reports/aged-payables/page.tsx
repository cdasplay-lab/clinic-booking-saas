...
import { Loader2, Download, ChevronDown, ChevronRight, AlertTriangle, Truck } from "lucide-react"
...
export default function AgedPayablesPage() {
  ...
  const load = useCallback(async (date: string) => {
    ...
    setError("")
    try {
      const res = await fetch(`/api/reports/aged-payables?asOf=${date}`)
  ...
  return (
    <div className="space-y-5" dir="rtl">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold">تقرير الذمم الدائنة المتقادمة</h1>
          <p className="text-sm text-gray-500">فواتير الموردين غير المسددة مصنّفة حسب عمر الدَّين</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" asChild>
            <a href={`/api/reports/aged-payables?asOf=${asOf}&export=xlsx`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <Card className="border-purple-200">
            <CardContent className="pt-4 pb-3">
              <p className="text-xs text-gray-500">إجمالي المستحق للموردين</p>
          ...
          <Card className="border-red-300">
            <CardContent className="pt-4 pb-3">
              ...
              <p className="text-xs text-red-500 mt-0.5">خطر تلف العلاقة مع المورد</p>
            </CardContent>
          </Card>
        </div>
      )}
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            تفصيل حسب المورد
            {data && <Badge variant="outline">{data.rows.length} مورد</Badge>}
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-16 text-gray-400">
              ...
              <p>لا توجد ذمم دائنة مستحقة بهذا التاريخ</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {data.rows.map((row: any) => (
                    <>
                      ...
                      {expanded.has(row.contact.id) && row.bills.map((bill: any) => (
                        <TableRow key={bill.id} className="bg-purple-50/40 text-sm">
                          <TableCell></TableCell>
                          <TableCell className="pr-6">
                            <Link href={`/dashboard/bills/${bill.id}`} className="text-purple-600 hover:underline font-mono text-xs">
                              {bill.number}
                            </Link>
                            <span className="text-gray-400 text-xs mr-2">
                              استحقاق: {new Date(bill.dueDate).toLocaleDateString("ar-SA")}
                            </span>
                          </TableCell>
                          {data.buckets.map((b: any) => (
                            <TableCell key={b.key} className="text-left">
                              {getBucketKey(bill.daysOverdue) === b.key ? (
                                <span className={`text-xs px-2 py-0.5 rounded-full ${BUCKET_COLORS[b.key]}`}>
                                  {fmt(bill.amount)}
                                </span>
                              ) : null}
                          ...
                          <TableCell className="text-left text-xs text-gray-500">
                            {bill.daysOverdue > 0 ? (
                              <span className="text-red-600">{bill.daysOverdue} يوم متأخر</span>
                            ) : (
                  ...
                  <TableRow className="bg-gray-100 font-bold border-t-2">
                    ...
                    <TableCell className="text-left text-purple-700">{fmt(data.totals.total)}</TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            </div>
