...
const statusConfig: Record<string, { label: string; variant: any }> = {
  ...
  PARTIAL:   { label: "تسليم جزئي",  variant: "warning" },
  DELIVERED: { label: "مسلّم",        variant: "success" },
  INVOICED:  { label: "تم الفوترة",  variant: "outline" },
  CANCELLED: { label: "ملغى",         variant: "destructive" },
}

const nextStatuses: Record<string, { value: string; label: string }[]> = {
  DRAFT:     [{ value: "CONFIRMED", label: "تأكيد الأمر" }, { value: "CANCELLED", label: "إلغاء" }],
  CONFIRMED: [{ value: "DELIVERED", label: "تسليم كامل" }, { value: "CANCELLED", label: "إلغاء" }],
  PARTIAL:   [{ value: "DELIVERED", label: "تسليم كامل" }],
  DELIVERED: [],
  INVOICED:  [],
  CANCELLED: [],
}
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link href="/dashboard/sales-orders"><ArrowRight className="h-4 w-4" /></Link>
        </Button>
        ...
        <div className="flex gap-2 flex-wrap justify-end">
          ...
          {canConvert && (
            <ConvertOrderButton orderId={order.id} orderType="so" />
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: "موعد التسليم",   value: order.expectedDate ? formatDateShort(order.expectedDate) : "—" },
