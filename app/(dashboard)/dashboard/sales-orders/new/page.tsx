...
export default function NewSalesOrderPage() {
  ...
  function applyProduct(i: number, productId: string) {
    ...
    if (!p) return
    const updated = [...items]
    updated[i] = { ...updated[i], description: p.name, unitPrice: String(Number(p.price)), total: Math.round(Number(p.price) * parseFloat(updated[i].quantity || "1") * 100) / 100 }
    setItems(updated)
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!form.contactId) { setError("يجب اختيار العميل"); return }
    ...
    <form onSubmit={handleSubmit} className="space-y-5 max-w-3xl">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" asChild>
          <Link href="/dashboard/sales-orders">
            <ArrowRight className="h-4 w-4 ml-1" /> أوامر البيع
          </Link>
        </Button>
        <span className="text-gray-300">/</span>
        <h1 className="text-xl font-bold">أمر بيع جديد</h1>
      </div>
      ...
      <Card>
        <CardContent className="pt-4">
          <div className="space-y-1.5">
            <Label>ملاحظات</Label>
            <Textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} rows={3} placeholder="أي ملاحظات إضافية..." />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex gap-3">
        ...
        <Button type="button" variant="outline" asChild>
          <Link href="/dashboard/sales-orders">إلغاء</Link>
        </Button>
      </div>
    </form>
  )
