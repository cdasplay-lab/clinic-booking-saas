...
export async function GET(req: NextRequest) {
  ...
  // All outstanding invoices (amountDue > 0, not cancelled/credited)
  ...
  for (const inv of invoices) {
    ...
    if (!contactMap.has(inv.contactId)) {
      contactMap.set(inv.contactId, {
        contact: inv.contact,
    ...
    const entry = contactMap.get(inv.contactId)!
    ;(entry as any)[bucket] = Math.round(((entry as any)[bucket] + amount) * 100) / 100
    entry.invoices.push({ id: inv.id, number: inv.number, date: inv.date, dueDate: inv.dueDate, daysOverdue, amount })
  }
  ...
  if (exportXlsx) {
    const header = ["العميل", ...BUCKETS.map((b) => b.label), "الإجمالي"]
    ...
    const ws = XLSX.utils.aoa_to_sheet([
      [`تقرير الذمم المدينة المتقادمة — بتاريخ ${asOf.toLocaleDateString("ar")}`],
      [],
      header,
      ...dataRows,
      [],
    ...
    XLSX.utils.book_append_sheet(wb, ws, "الذمم المدينة")
    return xlsxResponse(XLSX.write(wb, { type: "buffer", bookType: "xlsx" }), "aged-receivables")
  }
